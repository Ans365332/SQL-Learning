...
-- windows function / analytic function --> it is used to perform aggregate function
...
select employeename,saleamount,max(saleamount) over(),
min(saleamount) over() from employeesales;

-- partition by--> sepration based on similar value

select * ,max(saleamount) over(partition by employeename) from employeesales;

select *,max(saleamount) over() ,max(saleamount) over(partition by employeename) from employeesales;

-- over(order by) -- running sum (cummutative sum)
